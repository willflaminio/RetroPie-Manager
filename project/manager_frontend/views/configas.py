...
class RecalboxConfigAsFormView(FormView):
    """
    ...
    def dispatch(self, request, *args, **kwargs):
        self.config_filepath = settings.AS_SCRIPT_PATH
        self.config_content = self.get_config_file(self.config_filepath)
        
        self.backup_filepath = settings.AS_SCRIPT_BACKUP_PATH
    ...
    def get_success_url(self):
        return reverse('manager:configas')
            
    def get_form_kwargs(self):
        context = super(RecalboxConfigAsFormView, self).get_form_kwargs()
        context.update({
            'config_filepath': self.config_filepath,
            'backup_filepath': self.backup_filepath,
        })
    ...
    def form_valid(self, form):
        ...
        return super(RecalboxConfigAsFormView, self).form_valid(form)
